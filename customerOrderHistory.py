...
def filterDate():
    ...
    if filterDate in no:
        ...
        filteredByDate = joined \
            .select(['Ship_To','Order_Date']) \
            .where(lambda row: row['Order_Date'] > date) \
            .order_by('Ship_To')
        ...
        saveCustomersFiltered = input('Do you want to save "Last Order per Ship To after" '+ str(date)  +' yto a csv? (y/n) ')
...
# Join the two tables together by Ship_To Customer Number
...
joined = customers \
    .join(orders, 'Ship_To', 'Customer_No', inner=True) \
    .select(['Ship_To','Order_Date']) \
    .order_by('Order_Date', reverse=True) \
    .distinct('Ship_To') \
    .order_by('Ship_To')
